Shader "Custom/MouseTex"
{
    Properties
    {
        _MainTex ("Texture", 2D) = "white" {}
        _Mix ("Mix", Float) = 0
        _LookUpDistance ("Look Up Distance", Int) = 1
        //[HideInInspector] _mX("Mouse X", Float) = 0
        [HideInInspector] _mY("Mouse Y", Float) = 0
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                if(_LookUpDistance < 1) _LookUpDistance = 1;
                float2 texel = float2(
                ...
                float tx0y0 = (tex2D( _MainTex, i.uv + texel * float2( -1, -1 ) ).r //;
                            + tex2D( _MainTex, i.uv + texel * float2( -1, -1 ) ).b
                            + tex2D( _MainTex, i.uv + texel * float2( -1, -1 ) ).g) / 3;
                float tx0y1 = (tex2D( _MainTex, i.uv + texel * float2( -1, 0 ) ).r //;
                            + tex2D( _MainTex, i.uv + texel * float2( -1, 0 ) ).b
                            + tex2D( _MainTex, i.uv + texel * float2( -1, 0 ) ).g) / 3;
                float tx0y2 = (tex2D( _MainTex, i.uv + texel * float2( -1, 1 ) ).r //;
                            + tex2D( _MainTex, i.uv + texel * float2( -1, 1 ) ).b
                            + tex2D( _MainTex, i.uv + texel * float2( -1, 1 ) ).g) / 3;

                float tx1y0 = (tex2D( _MainTex, i.uv + texel * float2( 0, -1 ) ).r //;
                            + tex2D( _MainTex, i.uv + texel * float2( 0, -1 ) ).b
                            + tex2D( _MainTex, i.uv + texel * float2( 0, -1 ) ).g) / 3;
                float tx1y1 = (tex2D( _MainTex, i.uv + texel * float2( 0, 0 ) ).r //;
                            + tex2D( _MainTex, i.uv + texel * float2( 0, 0 ) ).b
                            + tex2D( _MainTex, i.uv + texel * float2( 0, 0 ) ).g) / 3;
